...
set_nvim_cat_flavour() {
    ...
    *)
    ...
    for SOCK in $XDG_RUNTIME_DIR/nvim.sock.*; do
            nvim --server $SOCK --remote-send "<CMD>:Catppuccin ${selected}<CR>"
    done

    sed -i "/flavour/c\ \ \ \ flavour = \"${selected}\"," ~/.config/nvim/lua/catp-conf.lua
}
